...
### Security schemes
- `core.model.SecurityScheme` covers the three types whose shape is flat strings — `apiKey`, `http`, `openIdConnect`. **`oauth2` is deliberately not editable**: it nests a `flows` object and is its own piece of work. A loaded oauth2 scheme is listed, is readable through the facade, and round-trips untouched; `SecuritySchemePane` shows it an explanation instead of a form and gives it **no type picker**, so it cannot be converted into something else by accident. It can still be deleted.
